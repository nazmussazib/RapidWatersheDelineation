...
def PreProcess_TauDEM_for_On_Fly_WatershedDelineation(
        input_dir_name,
        watershed_file,
        watershed_id_file,
        watershed_coast,
    ...
    if os.path.isfile(coast_file):
        with fiona.open(coast_file) as source:
            ...
            for f in source:
                dest = output_dir1
                outfile = os.path.join(
                    dest, "subwatershed_coast_%s.shp" %
                    f['properties']['GRIDCODE'])
                with fiona.open(outfile, 'w', **meta) as sink:
                    sink.write(f)
...
# # create complimentary watershed ( if exists)  for each subwatershed
    with fiona.open(infile) as source:
        ...
        for f in source:
            ID_complimentary = np.asarray(
                complementary_gagewatershed(
                    watershed_id_file, int(
                        f['properties']['GRIDCODE'])))
            ...
            if(Number_contribute_subwatershed == 0):
                ...
                inputfile = os.path.join(
                    source_dir, "subwatershed_" + str(f['properties']['GRIDCODE']) + ".shp")
                outputfile = 'Full_watershed' + \
                    str(f['properties']['GRIDCODE']) + '.shp'
                with collection(inputfile, "r") as input:
                    ...
                    with collection(
                        ...
                        for f in input:
                            shapes.append(
                                shape(f['geometry']).buffer(.000000001))
                ...
                for i in range(0, Number_contribute_subwatershed):
                    ...
                    os.chdir(source_dir)
                    file = os.path.join(
                        source_dir, "subwatershed_" + str(int(Real_ID_Complimentary[i])) + ".shp")
                    files.append(file)
                outputfile_WD = complimentary_subwatershed_file + \
                    str(f['properties']['GRIDCODE']) + 'WD'
                sub_water_file = os.path.join(
                    source_dir, "subwatershed_" + str(f['properties']['GRIDCODE']) + ".shp")
                ...
                for ff in files:
                    ...
                    # make DSI as integer value other wise it will not work
                    w.records.extend(r.records())
                w.fields = list(r.fields)
                w.save(outputfile_WD)
                inputfile = 'Full_watershed' + \
                    str(f['properties']['GRIDCODE']) + 'WD' + '.shp'
                outputfile = 'Full_watershed' + \
                    str(f['properties']['GRIDCODE']) + '.shp'
                with collection(inputfile, "r") as input:
                    schema = input.schema.copy()
...
# # make directory for each of the subwatershed and then extract and keep subwatershed, complimentary watershed shapefile
    with fiona.open(infile) as source:
        ...
        os.chdir(source_dir)
        for f in source:
            dest = output_dir2 + "/Subwatershed" + \
                str(f['properties']['GRIDCODE'])
            os.mkdir(dest)
            sub = os.path.join(source_dir, "subwatershed_" +
                               str(f['properties']['GRIDCODE']) + ".*")
            com = os.path.join(source_dir, "Full_watershed" +
                               str(f['properties']['GRIDCODE']) + ".*")
    ...
    if os.path.isfile(coast_file):
        with fiona.open(coast_file) as source:
            ...
            os.chdir(source_dir)
            for f in source:
                dest = output_dir2 + "/Subwatershed_coast" + \
                    str(f['properties']['GRIDCODE'])
                os.mkdir(dest)
                sub = os.path.join(
                    source_dir, "subwatershed_coast_" + str(f['properties']['GRIDCODE']) + ".*")
                files_sub = glob.glob(sub)
                for file1 in files_sub:
                    if os.path.isfile(file1):
                        shutil.copy2(file1, dest)
...
# # extract streamraster,flow direction, watershed grid for each of the subwateshed and store in the subwatershed directory
    with fiona.open(infile) as source:
        for f in source:
            dir = output_dir2 + "/Subwatershed" + \
                str(f['properties']['GRIDCODE'])
            os.chdir(dir)
            inputfn = 'subwatershed_' + \
                str(f['properties']['GRIDCODE']) + '.shp'  # input file name
            complimentary_subwatershed_file = 'Full_watershed' + \
                str(f['properties']['GRIDCODE']) + '.shp'
            outputBufferfn = 'subwatershed_buffer' + \
                str(f['properties']['GRIDCODE']) + '.shp'  # input file buffer shapefile name
            ...
            main_dir = input_dir1
            flow_file = p_file
            subwshed_file = gw_file
            stream_file = src_file
            D8distance_file = dist_file
            flowdir = os.path.join(main_dir, flow_file)
            flow_out_file = os.path.join(
                dir, 'subwatershed_' + str(f['properties']['GRIDCODE']) + "p.tif")
            subwaterdir = os.path.join(main_dir, subwshed_file)
            subwater_out_file = os.path.join(
                dir, 'subwatershed_' + str(f['properties']['GRIDCODE']) + "w.tif")
            streamdir = os.path.join(main_dir, stream_file)
            stream_out_file = os.path.join(
                dir, 'subwatershed_' + str(f['properties']['GRIDCODE']) + "src1.tif")
            distancedir = os.path.join(main_dir, D8distance_file)
            ...
            command_flow = "gdalwarp -te " + xmin + " " + ymin + " " + xmax + " " + ymax + \
                " -dstnodata -32768 -cutline " + outputBufferfn + " -cl " + layer_name + " " + flowdir + " " + flow_out_file
            command_subw = "gdalwarp -te " + xmin + " " + ymin + " " + xmax + " " + ymax + " -dstnodata -32768 -cutline " + \
                outputBufferfn + " -cl " + layer_name + " " + subwaterdir + " " + subwater_out_file
    ...
    if os.path.isfile(coast_file):
        with fiona.open(coast_file) as source:
            for f in source:
                dir = output_dir2 + "/Subwatershed_coast" + \
                    str(f['properties']['GRIDCODE'])
                os.chdir(dir)
                inputfn = 'subwatershed_coast_' + \
                    str(f['properties']['GRIDCODE']) + '.shp'
                outputBufferfn = 'subwatershed_coast_buffer' + \
                    str(f['properties']['GRIDCODE']) + '.shp'  # input file buffer shapefile name
                bufferDist = Buffer_distance  # buffer distance
                # creating buffer shape file
                createBuffer(inputfn, outputBufferfn, bufferDist)
                ...
                main_dir = input_dir1
                flow_file = p_file
                stream_file = src_file_coast
                flowdir = os.path.join(main_dir, flow_file)
                flow_out_file = os.path.join(
                    dir, 'subwatershed_coast_' + str(f['properties']['GRIDCODE']) + "p.tif")
                streamdir = os.path.join(main_dir, stream_file)
                stream_out_file = os.path.join(
    ...
    with fiona.open(infile) as source:
        for f in source:
            dir = output_dir2 + "/Subwatershed" + \
                str(f['properties']['GRIDCODE'])
            os.chdir(dir)
            buffer_watershed = os.path.join(
                dir, "subwatershed_buffer" + str(f['properties']['GRIDCODE']) + ".*")
    ...
    if os.path.isfile(coast_file):
        with fiona.open(coast_file) as source:
            for f in source:
                ...
                os.chdir(dir)
                buffer_watershed = os.path.join(
                    dir, "subwatershed_coast_buffer" + str(f['properties']['GRIDCODE']) + ".*")
